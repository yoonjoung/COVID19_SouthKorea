...
#####__Q 10. From "social distancing" to "distancing in daily life": keeping an eye on new cases (NEW)__
...
* In Gyungbuk, including Daegu city, `r incidenceGB` people in 100,000 population have been infected - compared to below 7 per 100,000 population in all other regions.  
...
dtaforpaper <- dtabyage%>%filter(latest==TRUE)%>%
    select(age, agegroup, confirmedcases, pop10, incidence)
...
The graph below shows trends of mortality by age group - starting from March 3rd. Mortality among those 80 and above has increased continuously. __As of `r latestbyage`, `r mortality80`% of people in their 80s who have confirmed COVID have died, increased from `r mortality80initial`% on `r initialbyage`__. This increase is not surprising, considering the typical clinical timeline and prognosis of COVID infection. A more important question is _if and when_ the mortality rate will level off, as this epidemic progresses and a bigger proportion of population is affected.  
...
subplot(fig50, fig60, fig70, fig80, shareY = TRUE) %>%
        layout(
            ...
            yaxis = list(title = "Mortality (per 100 confirmed cases)",
