...
#make sure the columns are all numeric 
colnames(dta[,6:24])
for (i in 6:24){
...
dtabyregion<-dta %>%
    ...
    mutate(
        gg=sum(seoul, incheon, gyeonggi), 
        gw=gangwon,
        cb=chungbuk,
        cn=sum(daejeon, sejong, chungnam),   
        gb=sum(daegu, ulsan, gyeongbuk),            
        gn=sum(busan, gyeongnam),            
        jb=jeonbuk,          
        jn=sum(gwangju, jeonnam),           
        jj=jeju,             
        airport=airportscreening, 
        TOTAL=sum(gg,gw,cn,cb,gb,gn,jb,jn,jj,airport)
    ) 


table(dtabyregion$status)
table(dtabyregion$date, dtabyregion$status)
temp<-dtabyregion %>% group_by(date) %>% summarize(count=n())
nrow(temp)
...
#####__Q 8 Is COVID spreading faster in some age groups than others?__ 
```{r}
...
temp<-dtabyage%>%select(date, agegroup, incidence)%>%filter(agegroup=="80+")%>%filter(date=="2020-03-16")
incidence80_0316<-mean(temp$incidence)

temp<-dtabyage%>%select(date, agegroup, incidence)%>%filter(agegroup=="80+")%>%filter(date=="2020-03-26")
incidence80_0326<-mean(temp$incidence)
 
```

As I saw overall mortality going up steadily (See Question 7.1), I wanted to know if COVID is spreading more rapidly among those who would have higher mortality risk (e.g., older people). One way to answer that question is to see if infection incidence rate (number of confirmed cases per population) increases more rapidly in one age group than others. 

In all age groups, there was a rapid increase in early March, when the country had a huge and acute outbreak. Following that, however, __the infection rate has increased more rapidly among those who are 80 and older than in any other age groups (red line in the below figure)__: from `r incidence80_0303` per 100,000 people on March 3rd when detailed data by age were released first, to `r incidence80_0316` on March 16th, and `r incidence80_0326` on March 26th. This may be due to recent outbreaks in nursing homes.  
...
fig <- plot_ly(dta, x=~date, y=~incidence0, name = "0-9", 
    ...
    layout(
        title ="Trends of incidence rate by age group",
...
fig <- subplot(fig50, fig60, fig70, fig80) %>%
        layout(
            title ="Trends of mortality by age group",
