...
```{r plotnewcasesbyregion,  fig.align="left", out.width="600px"}
#results="asis",

dta<-dtabyregion
plot_ly(dta, x=~date, y=~newcases, 
        type = 'bar', color= ~region ) %>%
        layout(
            title = c("Trend of daily new confirmed COVID-19 cases"),
...
dta<-dtabyregion%>%filter(region=="gg")
plot_ly(dta, x=~date, y=~newcases, type = 'bar' ) %>%
        layout(
            title = c("Trend of daily new confirmed COVID-19 cases"),
            xaxis = list(title = "Date",  
                         autotick = FALSE,
...
dta<-dtabyregion%>%filter(region=="gb")
plot_ly(dta, x=~date, y=~newcases, type = 'bar' ) %>%
        layout(
            title = c("Trend of daily new confirmed COVID-19 cases"),
            xaxis = list(title = "Date",  
...
dta<-dtabyregion%>%filter(region=="airport")
plot_ly(dta, x=~date, y=~newcases, type = 'bar' ) %>%
        layout(
            title = c("Trend of daily new confirmed COVID-19 cases"),
            xaxis = list(title = "Date",  
