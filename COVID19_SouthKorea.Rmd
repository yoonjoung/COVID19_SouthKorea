...
subplot(fig1, fig2, nrows=1) %>%
        layout(  title ="Total number of cases and cumulative incidence rate by region", 
                 showlegend = FALSE )
```

```{r,  results='asis', fig.align="left", out.width="800px"}
dtafig<-dtabyregion%>%filter(cfr>0)

fig1<-plot_ly(dtafig, x=~date, y=~cfr, 
        type = 'scatter', mode = 'lines', color= ~region , 
        colors = c("dodgerblue3", "blue", "red",  "dodgerblue1") ) %>%
        layout(
            title = c("Trend of COVID-19 mortality by region"),
            xaxis = list(title = "Date",  
                         autotick = FALSE,
                         showticklabels = TRUE, 
                         tickfont = list(size=10), 
...
dtafig<-dtabyregion%>%filter(region!="Airport")%>%
    ...
    mutate(region3="Other",
           region3=ifelse(region=="Gyeonggi","Gyeonggi",region3),
           region3=ifelse(region=="Gyungbuk","Gyungbuk",region3))%>%
    select(-region)%>%
    group_by(date, region3)%>%summarize_all(funs(sum))%>%
...
For this question, let's focus on __the number of new cases by day__. For about a month, only 30 cases were confirmed - mostly in/near Seoul. However, _starting from the case No. 31 (a super transmitter who was confirmed on February 18)_, the number of new cases exponentially increased - primarily in Daegu and a region surrounding it but throughout the country. For 10 days, more than 400 new cases were confirmed everyday, with a peak of 909 new cases on February 29. More recently, the number of new cases started to decrease, and it appears to remain stable (see below). __Still, as of `r latestsummary`, a total of `r totalcases` people have been infected. It means `r totalincidence` people have been infected out of 100,000 population since beginning of the epidemic__. 
