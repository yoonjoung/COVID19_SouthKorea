...
View(dtabyregiontotal)

dtabyregion<-dtabyregiontotal %>% 
    gather(region, cases, gg:airport, convert = FALSE) %>% 
    arrange(region, date) 
    #total confirmed cases by region & date
View(dtabyregion)

    colnames(dtabyregiontotal[,3:12])
    for (i in 3:12){
        for (j in 2:nrow(dtabyregiontotal)){
            dtabyregiontotal[j,i]=dtabyregiontotal[j-1,i]
            }
        }
    tail(dtabyregiontotal, 20)

dtabyregion2<-dtabyregiontotal %>% 
    gather(region, prev, gg:airport, convert = FALSE) %>% 
    arrange(region, date)   
    #total new cases by region & date
head(dtabyregion2,20)    

dtabyregion<-left_join(dtabyregion, dtabyregion2, by = c("date", "region"))
...
temp<-dtabyage%>%select(date, agegroup, incidence)%>%filter(agegroup=="80+")%>%filter(date=="2020-03-19")
incidence80_0319<-mean(temp$incidence)

temp<-dtabyage%>%select(date, agegroup, incidence)%>%filter(agegroup=="80+")%>%filter(date=="2020-03-29")
incidence80_0329<-mean(temp$incidence)
...
In all age groups, there was a rapid increase in early March, when the country had a huge and acute outbreak _(sorry, it's not very clear in the figure, because the age-specific trend data starts from March 3rd)_. Following that, however, __the infection rate has increased more rapidly among those who are 80 and older than in any other age groups (red line in the below figure)__: from `r incidence80_0303` per 100,000 people on March 3rd when detailed data by age were released first, to `r incidence80_0319` on March 19th, and `r incidence80_0329` on March 29th. This may be due to recent outbreaks in nursing homes.  
